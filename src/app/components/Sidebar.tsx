...
type MenuItem = {
  ...
  badge?: number;
  /**
   * Keeps the entry out of the nav without deleting it. The route and the page
   * still exist and still work — flip this off to bring the item back.
   */
  hidden?: boolean;
};
...
function MenuItemComponent({ 
  ...
  return (
    <button
      onClick={onClick}
      className={`
        group relative w-full flex items-center gap-3 px-3 py-2.5 rounded-[8px] transition-all duration-300
        ${isActive 
    ...
    >
      ...
      {!isCollapsed && (
        <>
          ...
          {badge && (
            <span className={`px-2 py-0.5 rounded-[8px] text-xs ${isActive ? 'bg-white/20' : 'bg-[#2A96A8]'} text-white`}>
              {badge}
            </span>
          )}
        </>
      ...
      {isCollapsed && (
        <div className="absolute left-full ml-2 px-3 py-1.5 bg-white text-[#092E3F] text-sm rounded-[8px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-50 shadow-xl">
          {label}
          {badge && (
            <span className="ml-2 px-1.5 py-0.5 rounded-[8px] bg-[#2A96A8] text-white text-xs">
              {badge}
            </span>
          )}
        </div>
  ...
  const menuSections: MenuSection[] = [
    {
      items: [
        ...
        { id: 'incidents', label: 'Incidents (A)', icon: AlertTriangle, badge: 3 },
        { id: 'incidents-b', label: 'Incidents (B)', icon: AlertTriangle },
        // Parked, not retired — the frozen pre-Resolve snapshot. Set hidden to
        // false (or drop the flag) to put it back in the nav.
        { id: 'incidents-backup', label: 'Incidents (Backup)', icon: AlertTriangle, hidden: true },
        ...
        {!isCollapsed ? (
          <>
            ...
            <button
              ...
              className="w-8 h-8 rounded-[8px] bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-all duration-200"
            >
          ...
          <div className="relative group">
            ...
            <button
              onClick={() => setIsCollapsed(!isCollapsed)}
              className="absolute inset-0 flex items-center justify-center w-8 h-8 rounded-[8px] bg-white/10 hover:bg-white/20 text-white opacity-0 group-hover:opacity-100 transition-all duration-200"
            >
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
        <nav className="space-y-2">
          {menuSections.map((section, index) => {
            ...
            // Hiding the last item in a section would otherwise leave its
      ...
      <div className="p-3 border-t border-white/10">
        {!isCollapsed ? (
          <div className="space-y-2">
            {/* User Profile Card */}
            <div className="p-3 rounded-[8px] bg-white/5 hover:bg-white/10 transition-all duration-200 cursor-pointer group">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 p-2.5 rounded-[8px] bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition-all duration-200 flex items-center justify-center gap-2"
                title="Settings"
              >
          ...
          <div className="space-y-2">
            ...
            <button className="relative group w-full flex justify-center">
              ...
              <div className="absolute left-full ml-2 px-3 py-2 bg-[#092E3F] text-white text-sm rounded-[8px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-50 shadow-xl border border-[var(--stroke)]/10">
            ...
            <button
              onClick={() => onNavigate('settings')}
              className="relative group w-full p-2.5 rounded-[8px] bg-white/5 hover:bg-white/10 text-white/70 hover:text-white transition-all duration-200 flex items-center justify-center"
              title="Settings"
            >
              <Settings className="w-4 h-4" />
              <div className="absolute left-full ml-2 px-3 py-1.5 bg-[#092E3F] text-white text-sm rounded-[8px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 whitespace-nowrap z-50 shadow-xl border border-[var(--stroke)]/10">
                Settings
              </div>
            </button>
